...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_tokens) {
    /* Fill this function */
    char *token;
    token = (char *)malloc(sizeof(char *) * MAX_DIR_LIST_SIZE);
   ...
   // current_dir = root_dir;
	...
	is_create = false;
	strcpy(token, token_list[i]);
        child_dir = find_create_dir(current_dir, token, &is_create);
	if (is_create) {
    ...
    is_create = false;
    strcpy(token, token_list[num_tokens - 1]);
    current_file = find_create_file(current_dir, token, &is_create);
    if (is_create) {
	    current_dir->file_list[current_dir->num_files++] = current_file;
    }
    return;
}
